...
parser.add_argument('--white_patch_roughness',type=float,help='white patch  roughness')
parser.add_argument('--black_patch_roughness',type=float,help='black patch  roughness')
parser.add_argument('--black_patch_retroreflectivity',type=float,help='black patch retroreflectivity')
parser.add_argument('--white_patch_retroreflectivity',type=float,help='white patch retroreflectivity')
...
bpy.data.materials['Material.005'].node_tree.nodes["Principled BSDF.003"].inputs["Roughness"].default_value = args.black_patch_roughness # Adjust roughness of black plastic

bpy.data.materials['Material.005'].node_tree.nodes["Principled BSDF.004"].inputs["Roughness"].default_value = args.white_patch_roughness # Adjust roughness of white plastic

bpy.data.materials['Material.005'].node_tree.nodes['Value.001'].outputs[0].default_value = args.black_patch_retroreflectivity # Adjust retroreflectivity of black plastic

bpy.data.materials['Material.005'].node_tree.nodes['Value.003'].outputs[0].default_value = args.white_patch_retroreflectivity # Adjust retroreflectivity of white plastic
...
if 1:
    ...
    timm_model = timm.create_model(kernel_type, pretrained=False)
    try:
    ...
    data = torch.load(f'<PATH_TO_PROJECT_ROOT>/blender_file/traffic_sign_classifiers/models_60/{kernel_type}_{DATA}_best_loss.pth', map_location=torch.device('cpu'))
    timm_model.load_state_dict(data['model'])
    map_models[kernel_type] = timm_model.to(device), data['classes']
    classes_GTSRB = data['classes']
US_SL65 = 26
...
# inference
def inference_ae(path,export_log=False,N=10,best = False,ground_truth=US_SL65):
    ...
    at_image =at_image_orig =  stop_sign[y1:y2, x1:x2]
    
    if best:
        
    
        cv2.imwrite(f"{path.replace('.png','')}-cropped.png",at_image)
    ...
    for i in range(N):
        ...
        blightness_factor = np.random.uniform(1.0,1.75)
        ...
        for k, (model, classes_GTSRB) in map_models.items():
            ...
            prob_of_stopsign = pred[0][ground_truth]
            prob = pred.max()
...
dir = f"<PATH_TO_PROJECT_ROOT>/blender_file/render/optuna/SL65/{args.kernel_type}/{args.patch_name}/{args.max_patch_size}/SL65_{args.patch_name}_{time.strftime('%Y%m%d%H%M%S', time.localtime())}"
...
def objective(trial):
    ...
    y1 = trial.suggest_uniform('y1', -1.5,1.1)
    z1 = trial.suggest_uniform('z1', 1.2, 4.7)
    ...
    try:
        ...
        prob,stopsign_prob = inference_ae(file_path,export_log=True,N=5,ground_truth=US_SL65)
        ...
        file_path = save_picture(dir,best = False)
        _, day_prob = inference_ae(file_path,export_log=True,N=1,ground_truth=US_SL65)
...
def test(params):
    ...
    print(f"ground_truth ({classes_GTSRB[US_SL65]}): {prob[US_SL65]}")
    ...
    data = {
        ...
        "Best attack result num": f"{label}",
        "Best attack result": f"{classes_GTSRB[label]} ({prob.max():.2f})",
        "Ground truth num": f"{US_SL65}",
        "Ground truth": f"{classes_GTSRB[US_SL65]} ({prob[US_SL65]:.2f})",
