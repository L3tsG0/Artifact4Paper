...
HOME_DIR = _normpath(Path(os.environ.get("HOME_GO") or os.environ.get("HOME") or str(Path.home())))
DESKTOP_DIR = HOME_DIR / "Desktop"
DEFAULT_SAVE_DIR = HOME_DIR / "Code" / "yolov5" / "Yolov5_AE" / "Sato" / "blender" / "render" / "optuna"
...
image_path = Path(args.background_image_path).expanduser()
if not image_path.is_absolute():
    image_path = PROJECT_ROOT / image_path
image_path = _normpath(image_path)
image_ = bpy.data.images.load(os.fspath(image_path))
...
PATH_TO_IMAGE = HOME_DIR / "Code" / "yolov5" / "Yolov5_AE" / "Attacker" / "stopsign_img" / "Stopsign_cropped.png"
PATH_TO_IMAGE = HOME_DIR / "Code" / "yolov5" / "Yolov5_AE" / "Attacker" / "stopsign_img" / "SpeedLimit30.png"
image_size = 60
IMG_HEIGHT = 60
IMG_WIDTH = 60
...
model_path = PROJECT_ROOT / "blender_file" / "traffic_sign_classifiers" / "models_60" / f"SimpleCNN_{DATA}_best_loss.pth"
data = torch.load(os.fspath(model_path), map_location=torch.device('cpu'))
...
def inference(path,export_log=False):
    ...
    cv2.imwrite(at_image, os.fspath(DESKTOP_DIR / "crp2.jpg"))
    ...
    with open(DESKTOP_DIR / "test.log", "a")as f:
        f.write(text)
        f.write("\n")
...
def save_picture(savepath=DEFAULT_SAVE_DIR, best=False, RGBA=False) -> str:
    savepath = Path(savepath).expanduser()
    if not savepath.is_absolute():
        savepath = PROJECT_ROOT / savepath
    savepath = _normpath(savepath)
    ...
    bpy.context.scene.render.filepath = os.fspath(savepath / f"{time_}.png")
...
output_dir = PROJECT_ROOT / "blender_file" / "render" / "optuna" / "STOPSIGN" / str(args.patch_name) / str(args.max_patch_size)
output_dir = output_dir / f"STOPSIGN_{args.patch_name}_maxpatchsize_{args.max_patch_size}_{time.strftime('%Y%m%d%H%M%S', time.localtime())}"
output_dir = _normpath(output_dir)
os.makedirs(output_dir, exist_ok=True)

import logging
optuna.logging.get_logger("optuna").addHandler(logging.FileHandler(os.fspath(output_dir / "test.log")))
...
# set_reflector_size(reflector=REFLECTOR,w=REFLECTOR_W,h=REFLECTOR_H) # TODO: change reflector size here.
    ...
    try:
        file_path = save_picture(output_dir, best=False)
...
# set_reflector_size(reflector=REFLECTOR,w=REFLECTOR_W,h=REFLECTOR_H) # TODO: change reflector size here.
    file_path = save_picture(output_dir, best=True)
...
#        return -0.5
    ...
    with open(output_dir / "log.json", "a") as f:
        json.dump(data, f)
        f.write("\n")

    # Write the same content to output_csv_dir

    output_csv_dir = Path(args.output_csv_dir).expanduser()
    if not output_csv_dir.is_absolute():
        output_csv_dir = PROJECT_ROOT / output_csv_dir
    output_csv_dir = _normpath(output_csv_dir)
    os.makedirs(output_csv_dir, exist_ok=True)


    with open(output_csv_dir / "log.json", "a") as f:
        json.dump(data, f)
        f.write("\n")
            
    save_picture(output_dir, RGBA=True)
