...
dir = f"<PATH_TO_PROJECT_ROOT>/blender_file/render/optuna/hiding/SL65/{args.patch_name}/{args.max_patch_size}/SL65_{args.patch_name}_maxpatchsize_{args.max_patch_size}_{time.strftime('%Y%m%d%H%M%S', time.localtime())}"
...
def save_picture(savepath = "<PATH_TO_HOME_GO>/Code/yolov5/Yolov5_AE/Sato/blender/render/optuna",best=False,RGBA=False)->str:
    ...
    if RGBA:
        ...
        bpy.context.scene.render.filepath = f"{savepath}/render/{time_}.png"
    ...
    try:
        def _score(render_output_path, N):
            ...
            for i in range(N):
                ...
                score = run_yolov5_detection(f"{new_filename}",filter_name="R2-165")
...
def objective(trial):
    ...
    confidence = render_and_get_confidence(N=10)
#    print(confidenmce)
...
def test_params(params):
    ...
    confidence = render_and_get_confidence(N=1,RGBA = True)
